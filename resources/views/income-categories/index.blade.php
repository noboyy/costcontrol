...
@section('breadcrumb')
    ...
    <span class="sep">/</span>
    <span class="current">Kategori Pendapatan</span>
@endsection
...
<div class="page-header">
    <div>
        <h2>Kategori Pendapatan</h2>
        <p>Master kategori untuk mengelompokkan tipe pendapatan</p>
    </div>
    <div class="page-actions">
        <a href="{{ route('income-types.index') }}" class="btn btn-outline"><i class="bi bi-tags"></i> Tipe Pendapatan</a>
                ...
                <tbody>
                    @forelse($categories as $cat)
                        ...
                        <tr data-search="{{ strtolower($cat->kode.' '.$cat->nama) }}">
                            ...
                            <td class="text-end">
                                @if($count > 0)
                                    <a href="{{ route('income-types.index') }}#{{ $cat->kode }}" class="badge badge-blue">{{ $count }} tipe</a>
                                @else
                            ...
                            <td class="text-end">
                                <div class="btn-group">
                                    <button type="button" class="btn btn-xs btn-outline btn-icon" title="Edit" onclick="openModal('edit{{ $cat->id_income_category }}')"><i class="bi bi-pencil"></i></button>
                                    <form action="{{ route('income-categories.delete', $cat->id_income_category) }}" method="POST" data-confirm="{{ $count > 0 ? 'Masih ada tipe yang memakai kategori ini. Tetap coba hapus?' : 'Hapus kategori ini?' }}">
                                        @csrf
                        ...
                        <div class="modal-backdrop" id="edit{{ $cat->id_income_category }}">
                            <div class="modal">
                                <form action="{{ route('income-categories.update', $cat->id_income_category) }}" method="POST">
                                    @csrf
                                    <div class="modal-header">
                                        <h3>Edit Kategori</h3>
                                        <button type="button" class="modal-close" onclick="closeModal('edit{{ $cat->id_income_category }}')">×</button>
                                    </div>
                                    <div class="modal-body">
                                        ...
                                        <div class="form-row">
                                            <div class="form-group">
                                                ...
                                                <div class="form-hint">Ubah kode = sinkron otomatis ke tipe pendapatan</div>
                                            </div>
                                            <div class="form-group">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-outline" onclick="closeModal('edit{{ $cat->id_income_category }}')">Batal</button>
